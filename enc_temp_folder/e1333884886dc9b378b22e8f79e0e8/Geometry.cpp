...
LIGHT::LIGHT(V3 src, V3 direct, COLOR sh, float a) {
	src.normalize();
	direct.normalize();
	source = src;
	direction = direct;
...
bool LIGHT::is_subject(V3& point) {
	...
	float area = (delta ^ direction).length();
	if (area > thresold) return false; // falls within the fov
	float dot = delta * direction;
	return dot >= 0.0f; // angle is acute
}
